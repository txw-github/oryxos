...
![OryxOS 系统架构图](images/oryxos-architecture.svg)
...
1. 第一部分 system prompt（Profile 的 identity prompt 加 Bootstrap 文件加 Skill 文件内容，统一由下面讲的 `ContextLoader` 提供）；
2. 第二部分 Memory 注入（会话历史加长期记忆，由 `MemoryService` 提供）；
3. 第三部分对话历史（按 maxHistoryTurns 截断后的 Session messages）；
4. 第四部分当前 Profile 可用的 Tool 列表（按 Function Calling 格式）。
...
| 周次 | 任务内容 | 可演示成果 |
|---|---|---|
| 第一周（3 小时）：核心能力一加二（对接 LLM 加 ReAct 循环） | 搭 Maven 多模块骨架（9 个模块）、`oryxos init`、Profile YAML 解析；`ProviderService` 包装 Spring AI Alibaba（先跑通 DeepSeek 或 Kimi，含 provider name 映射）；`ReActLoop` 加 `PromptBuilder` 加 `ToolExecutor`、一个内置 HTTP Tool、`CliChannel`；Session 内存版（第四周加 `SQLite`） | `oryxos chat` 多轮对话，Agent 调 HTTP Tool 完成简单任务。 |
| 第二周（3 小时）：核心能力三加四（Memory 加 Tool） | `MemoryService` 三层门面加 `LongTermMemory`（MEMORY.md 读写）、save_memory 加 recall_memory；`PromptBuilder` 加 Memory 注入；文件 Tool 加 Shell Tool（带白名单）、`McpClientService`（连接外部 MCP server）；`ContextLoader` 加载 SKILL.md | Agent 记住偏好并后续用到，调本地文件读写、调外部 MCP server 完成跨工具任务。 |
| 第三周（3 小时）：核心能力五 Web Service | `WebServer`（Spring MVC 加 virtual thread）、六个 ApiController 的核心 10 个端点；`GlobalExceptionHandler`、`ConfigLoader`（配置与密钥加载） | 外部系统通过 10 个 REST 端点完整调用 OryxOS。 |
| 第四周（3 小时）：多 Agent 演示加工程化收尾 | 多 Agent 演示（两个不同 Profile 的 Agent 在同一实例并存）；Session 持久化到 `SQLite`（含 `tool_invocations`、`llm_calls` 写入）；`ContextLoader` 的 Bootstrap 加载、`Picocli` 12 个命令完整、结构化日志；项目主页（VitePress 或类似） | 多 Agent 并存可用，CLI 体验流畅，Bootstrap 影响 Agent 行为，Session 跨重启恢复，主页可访问。 |
